...
**This build has no `ANTHROPIC_API_KEY` and ships running on the
template fallback.** The LLM explanation layer (M7) is fully built and
tested against real API failure modes — see "The explanation layer (M7)"
below — but no key is available in this environment, so every
explanation in the live demo is generated by the deterministic template
path, not by Claude. That is the honest state of this build, not a
